...
// Type: Qios.DevSuite.Components.QCompositeMenuControlListener
...
{
  internal sealed class QCompositeMenuControlListener : NativeWindow
  {
    private QCompositeMenu m_oCompositeMenu;
    ...
    public QCompositeMenuControlListener(
      QCompositeMenu compositeMenu,
      Control control,
      bool designMode)
    {
      ...
      this.m_oCompositeMenu = compositeMenu;
      this.AssignHandle();
    }
    ...
    protected override void WndProc(ref Message m)
    {
      if (m.Msg == 123)
      {
        ...
        if (!this.m_oCompositeMenu.IsWindowVisible)
          this.m_oCompositeMenu.ShowMenu(location, (IWin32Window) this.m_oControl.TopLevelControl, QCompositeActivationType.None);
        if (this.m_oCompositeMenu.SuppressDefaultContextMenu)
          return;
      }
      base.WndProc(ref m);
    }

    private void Control_Disposed(object sender, EventArgs e) => this.m_oCompositeMenu.RemoveListener(this.m_oControl);
